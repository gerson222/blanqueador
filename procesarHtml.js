...
// Obtiene clases y etiquetas cuando ambos están presentes en la misma regla CSS
function obtenerClasesYEtiquetasConColorEnCombinacion(document) {
    const clasesYEtiquetasABorrar = [];
    const styleTags = document.querySelectorAll('style');
    ...
    styleTags.forEach(tag => {
        const cssText = tag.textContent;
        coloresParaEliminar.forEach(color => {
            // Regex para encontrar todas las clases y etiquetas con color específico en la misma regla
            const regex = new RegExp(
                `([\\w-]+|[a-zA-Z]+)\\s*,?\\s*([\\w-]+|[a-zA-Z]+)?\\s*{[^}]*color\\s*:\\s*${color};[^}]*}`, 
                'g'
            );
...
function eliminarEtiquetasYClasesConColor(document) {
    ...
    const clasesYEtiquetasConColor = obtenerClasesYEtiquetasConColorEnCombinacion(document);

    console.log('Clases con color:', clasesYEtiquetasABorrar);
    console.log('Etiquetas con color:', etiquetasConColor);
    console.log('Clases y Etiquetas con color (en combinación):', clasesYEtiquetasConColor);
    ...
    document.querySelectorAll('[class]').forEach(tag => {
        ...
        // Verifica si todas las clases son para eliminar
        const allClassesForDeletion = classesToDelete.length > 0 && !classes.some(clase => !clasesYEtiquetasABorrar.includes(clase));
        if (allClassesForDeletion) {
            tag.remove();
        }
    });
    ...
    document.querySelectorAll('*').forEach(tag => {
        ...
        const hasColorClass = Array.from(tag.classList).some(clase => !/^s\d{1,2}$/.test(clase) && !clasesYEtiquetasABorrar.includes(clase));
        if (etiquetasConColor.includes(tagName) && !hasColorClass) {
            tag.remove();
        }
    });
